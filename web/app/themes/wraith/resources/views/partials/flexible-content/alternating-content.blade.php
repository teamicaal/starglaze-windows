					...
					<div class="flex flex-col items-start justify-center h-full lg:w-[90%] w-full {{ $i % 2 == 0 ? 'ml-auto' : ' ' }}">
						...
						@if( have_rows('buttons') )
							<div class="block w-full sm:flex sm:flex-wrap mt-8">
								...
								@endphp
								<a href="{{ $link }}"
								class="btn {{ $j == 1 ? 'bg-transparent text-[#343338] text-medium border border-primary hover:scale-105 transition-all ease-in-out duration-500 text-center block md:inline-block mb-4 md:mr-6' : 'bg-primary hover:scale-105 transition-all ease-in-out duration-500 text-white border-0 text-center block md:inline-block' }}">{!!
								$label !!}</a>
								@endwhile
							</div>
        				@endif
